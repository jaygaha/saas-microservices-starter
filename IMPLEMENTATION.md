...
- `make seed`: load demo data (idempotent: safe to re-run).
...
`auth-service` uses **sqlc** to generate type-safe Go from hand-written SQL: you edit SQL, not Go, for queries.
...
- **Config:** `auth-service/sqlc.yaml`: `schema:` points at `infra/database/migrations` (one source of truth); overrides map `citext → string` and `uuid → google/uuid`.
...
Use a demo data 
- `make seed`: add preconfigured data (idempotent: safe to re-run).
